...
void saveUsersToFile(const User users[], int numUsers) {
 ...
 // Write user data from the array to the file
...
void viewBakeryItems(const vector<BakeryItem>& items) {
    ...
    if (items.empty()) {
        ...
        cout<<"\n\t\t";
	    cout<<CYAN_COLOR<<"___________________________________________ "<<RESET_COLOR;
	    cout<<"\n\t\t";
	    cout<<CYAN_COLOR<<"|  Item ID.  |   Item Name   |  Price  "<<RESET_COLOR;
	    cout<<"\n\t\t";
	    cout<<CYAN_COLOR<<"---------------------------------------------\n"<<RESET_COLOR;
        //cout <<PURPLE_COLOR<< "Bakery Items:\n"<<RESET_COLOR;
        //cout << left << setw(30) << "item id" << setw(30) << "item name" << setw(30) << "item price" << endl;
     for (const BakeryItem& item : items) {
    ...
    while (true) {
        ...
        switch (cashierChoice) {
            case 1: {
                ...
                cout << "\n\t\t";
                cout <<CYAN_COLOR<< "______________________________________________ "<<RESET_COLOR;
                cout << "\n\t\t";
                cout <<CYAN_COLOR<< "|  Item ID.  |   Item Name   |  Price  "<<RESET_COLOR;
                cout << "\n\t\t";
            ...
            case 2: {
                if (order.items.empty()) {
                    ...
                    cout << GREEN_COLOR<< "Thank you for using the cashier system.\n"<<RESET_COLOR;
               ...
               if (!loggedInUser.username.empty()) {
                  ...
                  loggedInUser = {};
                    cout << GREEN_COLOR << "Logout successful.\n" << RESET_COLOR;
                     exit(0); // Specify an exit status code (0 for success)
             } else {
